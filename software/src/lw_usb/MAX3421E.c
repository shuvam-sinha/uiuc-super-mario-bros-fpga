...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE writeData[2];
	BYTE readBuffer[2];
	writeData[0] = reg | 0x02;
	writeData[1] = val;
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	Status = XSpi_Transfer(&SpiInstance, writeData, readBuffer, 2);
	if(Status != XST_SUCCESS) {
			xil_printf("SPI Device failed to write: %d\n", Status);
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	Status = XSpi_Transfer(&SpiInstance, writeData, buffer, nbytes + 1);
	if(Status != XST_SUCCESS) {
			xil_printf("ERROR: SPI write failed in MAXbytes_wr\n");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x00);
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	Status = XSpi_Transfer(&SpiInstance, sendData, recvData, 2);
	if(Status != XST_SUCCESS) {
			xil_printf("ERROR: SPI read failed in MAXreg_rd\n");
			return 0;
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x00);
	return recvData[1];
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE recvData[nbytes + 1];
	sendData[0] = reg;
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	Status = XSpi_Transfer(&SpiInstance, sendData, recvData, nbytes + 1);
	if(Status != XST_SUCCESS) {
			xil_printf("ERROR: SPI read failed in MAXbytes_rd\n");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x00);
	//{
	memcpy(data, &recvData[1], nbytes);
	//}
	return (data + nbytes);
}
